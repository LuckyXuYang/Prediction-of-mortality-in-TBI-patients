...
from step2to4_train_validate_inference.loader.img_mask_aug import data_aug
from step2to4_train_validate_inference.tnscui_utils.TNSUCI_util import char_color
...
class ImageFolder_new(data.Dataset):
	...
	def __getitem__(self, index):
		...
		image_o = image = Image.open(image_path).convert('L')
		GT_o = GT = Image.open(GT_path)

		if self.load_preseg:
			seg_path = self.seg_paths[index]
			seg_o = seg = Image.open(seg_path).convert('L')
