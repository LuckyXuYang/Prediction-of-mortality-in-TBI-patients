...
from step2to4_train_validate_inference.tnscui_utils.TNSUCI_util import char_color, GradualWarmupScheduler
...
class Solver(object):
	...
	def build_model(self):
		...
		# self.unet = smp.DeepLabV3Plus(encoder_name="efficientnet-b6", #efficientnet-b6
		# 							  encoder_weights='imagenet',  #imagenet
		# 							  in_channels=self.img_ch, classes=1)
		self.unet = smp.Unet(encoder_name='resnet18', encoder_weights='imagenet', in_channels=self.img_ch, classes=1)

		# from step2to4_train_validate_inference.segmentation_models_pytorch_4TorchLessThan120.transformerModel.factory import create_segmenter
		# net_kwargs = variant["net_kwargs"]
		# net_kwargs["n_cls"] = n_cls
		# self.unet = create_segmenter(net_kwargs)
		...
		# self.unet = smp.UResnet3p_cbam(block=smp.BottleNeck, layers=[3, 4, 6, 3], num_classes=1)
		...
		if self.lr_warm_epoch != 0 and self.lr_cos_epoch == 0: # 自适应
	...
	def test_tta(self, mode='train', unet_path=None):
		...
		# model pre for each image
		detail_result = []  # detail_result = [id, acc, SE, SP, PC, dsc, IOU]
		with torch.no_grad():  # 表明当前计算不需要反向传播，使用之后，强制后边的内容不进行图的构建
			for i, sample in enumerate(data_lodear):
				...
				for ii in range(SR.shape[0]):
					SR_tmp = SR[ii, :].reshape(-1)
					GT_tmp = GT[ii, :].reshape(-1)
					tmp_index = images_path[ii].split('\\')[-1]  #//xx.PNG
					tmp_index = int(tmp_index.split('.')[0][:])
