function [img_p, mask_p] = preprocess(img,mask,value_thresold,stage1_aim_size)
    ...
    x_hold_range = round(length(value_x)*[0.24/3, 2.2/3]); %round(length(value_x)*[0.24/3, 2.2/3]);
    y_hold_range = round(length(value_y)*[0.8/3, 1.8/3]);   %round(length(value_y)*[0.8/3, 1.8/3]);
